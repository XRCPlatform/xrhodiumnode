...
namespace NBitcoin.Tests
{
        ...
        // Require a BRhodium node running with the following BRhodium.conf file
        //server=1
        //rpcuser=rpcuser
    ...
    {
        public static NodeBuilderBRhodium Create([CallerMemberName]string caller = null, string version = "0.12.1")
        {
            ...
            return new NodeBuilderBRhodium(caller, path);
        }
        ...
        public NodeBuilderBRhodium(string root, string bitcoindPath)
        {
            this._Root = root;
        ...
        private readonly List<CoreNodeBRhodium> _Nodes = new List<CoreNodeBRhodium>();
        public List<CoreNodeBRhodium> Nodes
        {
            get
            {
                return _Nodes;
        ...
        public CoreNodeBRhodium CreateNode(bool start = false)
        {
            ...
            //{
            //}
            var node = new CoreNodeBRhodium(child, this);
            Nodes.Add(node);
            return node;
        }
        ...
        {
            if (!Process.GetProcesses().Any(p => p.ProcessName.Contains("BRhodium")))
                throw new NotSupportedException("BRhodium node is not running");
    ...
    public class CoreNodeBRhodium
    {
        private readonly NodeBuilderBRhodium _Builder;
        ...
        public CoreNodeBRhodium(string folder, NodeBuilderBRhodium builder)
        {
            ...
            ports = new int[2];
            FindPorts(ports);
            //ports[1] = Network.BRhodiumMain.RPCPort;
            //ports[0] = Network.BRhodiumMain.DefaultPort;
        }
        ...
        {
            return Node.Connect(Network.BRhodiumMain, "127.0.0.1:" + Network.BRhodiumMain.DefaultPort); //ports[0].ToString());
        }
