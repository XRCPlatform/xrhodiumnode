...
namespace NBitcoin.Tests
{
    public class pos_ProtocolTests
    {
        ...
        public void CanParseMessages()
        {
            ...
            {
                var message = Network.BRhodiumMain.ParseMessage(TestUtils.ParseHex(test.Message), test.Version);
        ...
        public void CanCancelConnection()
        {
            ...
            using (var builder = NodeBuilderBRhodium.Create())
            {
                ...
                try
                {
                    var client = Node.Connect(Network.BRhodiumMain, "127.0.0.1:" + node.ProtocolPort.ToString(), new NodeConnectionParameters()
                    {
        ...
        public void CanGetTransactionsFromMemPool()
        {
            ...
            using (var builder = NodeBuilderBRhodium.Create())
            {
                ...
                //node.Start();
                //node.Generate(101);
                //node.CreateRPCClient().SendToAddress(new Key().PubKey.GetAddress(Network.BRhodiumMain), Money.Coins(1.0m));
        ...
        public void CanDownloadBlock()
        {
            ...
            using (var builder = NodeBuilderBRhodium.Create())
            {
                ...
                {
                    Hash = Network.BRhodiumMain.GenesisHash,
