    ...
    public partial class SendingToAndFromManyAddressesSpecification : BddSpecification
    {
        ...
        private void two_connected_nodes()
        {
            this.nodes = this.nodeGroupBuilder
                .BRhodiumPowNode(NodeOne).Start().NotInIBD().WithWallet(WalletName, WalletPassword)
                .BRhodiumPowNode(NodeTwo).Start().NotInIBD().WithWallet(WalletName, WalletPassword)
                .WithConnections()
