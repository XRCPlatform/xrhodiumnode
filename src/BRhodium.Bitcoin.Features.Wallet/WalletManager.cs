    ...
    public class WalletManager : IWalletManager
    {
        ...
        public void LoadWalletsFromFiles()
        {
            ...
            foreach (Wallet wallet in wallets)
            {
                ...
                wallet.AccountsRoot.FirstOrDefault().LastBlockSyncedHash = this.chain.Genesis.Header?.GetHash();
        ...
        private void StoreStrippedWallets()
        {
            ...
            {
                var fileStorage = new FileStorage<Wallet>(Path.Combine(this.FileStorage.FolderPath, "block0"));
        ...
        public AddressBalance GetAddressBalance(string address, string walletName = null)
        {
            ...
            lock (this.lockObject)
            {
                Wallet wallet = this.GetWalletByAddress(address);             
            ...
            lock (this.lockObject)
            {
                ...
                foreach (TxOut utxo in transaction.Outputs)
                {
                    ...
                    if (this.addressByScriptLookup.ContainsKey(utxo.ScriptPubKey.Hash))
                    {
                        ...
                        this.AddTransactionToWallet(transaction, utxo, address, blockHeight, block, isPropagated);
        ...
        private void AddTransactionToWallet(Transaction transaction, TxOut utxo, WalletLinkedHdAddress walletLinkedHdAddress, int? blockHeight = null, Block block = null, bool isPropagated = true)
        {
            Guard.NotNull(transaction, nameof(transaction));
