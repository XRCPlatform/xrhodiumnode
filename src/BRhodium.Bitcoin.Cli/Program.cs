...
namespace BRhodium.Bitcoin.Cli
{
        ...
        public static void Main(string[] args)
        {
            try
            {
                ...
                if (optionList.Contains("-help") || optionList.Contains("--help") || string.IsNullOrWhiteSpace(command))
                {
                    ...
                    builder.AppendLine(" dotnet run <BRhodium.Bitcoin.Cli/BRhodium.Bitcoin.Cli.dll> [network-name] [options] <command> [arguments]");
                    builder.AppendLine();
                    builder.AppendLine("Command line arguments:");
                    builder.AppendLine();
                    builder.AppendLine("[network-name]                     Name of the network - e.g. \"BRhodium\", \"BRhodiummain\", \"BRhodiumtest\", \"bitcoinmain\", \"bitcointest\".");
                    ...
                    builder.AppendLine("-rpcport=<port>                    Connect to JSON-RPC on <port> (default for BRhodium: 26174 or default for Bitcoin: 8332)");
                    ...
                    builder.AppendLine();
                    builder.AppendLine("dotnet run BRhodium Wallet/history WalletName=testwallet - Lists all the historical transactions of the wallet called 'testwallet'.");
                    builder.AppendLine("dotnet run BRhodium getinfo -rpcuser=BRhodiumtestuser -rpcpassword=BRhodiumtestpassword -rpcconnect=127.0.0.3 -rpcport=26174 - Displays general information about the BRhodium node on the 127.0.0.3:26174, authenticating with the RPC specified user.");
                ...
                if (networkName.Contains("BRhodium"))
                {
                    defaultRestApiPort = 37221;
                    network = Network.BRhodiumMain;
                }
                else
                {
