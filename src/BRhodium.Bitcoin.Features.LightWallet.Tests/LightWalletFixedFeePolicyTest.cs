...
namespace BRhodium.Bitcoin.Features.LightWallet.Tests
{
    ...
    public class LightWalletFixedFeePolicyTest
    {
        ...
        [Fact]
        public void MinTxFee_OnBRhodiumNetwork_IsBRhodiumDefault()
        {
            ILoggerFactory loggerFactory = new LoggerFactory();
            NodeSettings nodeSettings = new NodeSettings(Network.BRhodiumTest);
            LightWalletFixedFeePolicy policy = new LightWalletFixedFeePolicy(loggerFactory, nodeSettings);
            Assert.Equal(new FeeRate(Network.BRhodiumTest.FallbackFee), policy.FallbackTxFeeRate);
        }
