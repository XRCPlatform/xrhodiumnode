...
namespace BRhodium.Bitcoin.Configuration.Logging
{
    /// <summary>
    ...
    public static class LoggingConfiguration
    {
        ...
        private static readonly Dictionary<string, string> keyCategories = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            //{ "lock", "" },
            //{ "mempoolrej", "" },
            { "net", $"{nameof(BRhodium)}.{nameof(Bitcoin)}.{nameof(Connection)}.*" },
            //{ "proxy", "" },
            //{ "prune", "" },
            //{ "rand", "" },
            //{ "reindex", "" },
            ...
            // Short Names
            { "configuration", $"{nameof(BRhodium)}.{nameof(Bitcoin)}.{nameof(Configuration)}.*" },
            { "fullnode", $"{nameof(BRhodium)}.{nameof(Bitcoin)}.{nameof(FullNode)}" }
        };
        ...
        private static void AddFilters(LogSettings settings = null, DataFolder dataFolder = null)
        {
            ...
            var defaultRule = new LoggingRule($"{nameof(BRhodium)}.{nameof(Bitcoin)}.*", NLog.LogLevel.Info, mainTarget);
            ...
            if (settings.DebugArgs.Any())
            {
                if (settings.DebugArgs[0] == "1")
                {
                    ...
                    defaultRule = new LoggingRule($"{nameof(BRhodium)}.{nameof(Bitcoin)}.*", NLog.LogLevel.Debug, mainTarget);
                }
                else
                {
                    ...
                    foreach (string key in settings.DebugArgs)
                    {
                        ...
                        {
                            // Allow direct specification - e.g. "-debug=BRhodium.Bitcoin.Miner".
                            category = key.Trim();
                        }
            ...
            if (settings != null)
            {
                if (settings.DebugArgs.Any())
                {
                    if (settings.DebugArgs[0] == "1")
                    {
                        ...
                        consoleLoggerSettings.Switches.Add($"{nameof(BRhodium)}.{nameof(Bitcoin)}", Microsoft.Extensions.Logging.LogLevel.Debug);
                    }
                    else
                    {
                        HashSet<string> usedCategories = new HashSet<string>(StringComparer.Ordinal);
